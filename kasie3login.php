<?php
    ...
    if(isset($_POST['submit'])){
        if(empty($_POST['nip']) || empty($_POST['password'])){
            ...
            $query = mysqli_query($conn, "SELECT * FROM kasie3login WHERE nip='$nip' AND password='$password'");
            $rows = mysqli_num_rows($query);
            if($rows == 1){
                header("Location: kasie3page.php");
            }else{
                $error = "NIP or Password is Invalid";
            }
            mysqli_close($conn);
...
<html lang=en>
    ...
    <body>
        <center>
            <b><h2>Login Kasie Data Statistik</h2></b><br>
            <form method="post">
                <table>
                    <tr>
